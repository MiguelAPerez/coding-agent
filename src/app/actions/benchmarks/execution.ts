...
import { benchmarks, benchmarkEntries, contextGroups, systemPrompts, systemPromptSets, benchmarkRuns } from "@/../db/schema";
...
export async function executeBenchmarkTask(benchmarkId: string): Promise<{ finished: boolean; throttled?: boolean; entry?: BenchmarkEntry }> {
    ...
    if (nextPendingEntry) {
        ...
        const matchDetails = (cg.expectations ? (typeof cg.expectations === 'string' ? JSON.parse(cg.expectations) : cg.expectations) : []).map((exp: { type: string; value: string }) => {
            let found = false;
            switch (exp.type) {
                case "contains":
