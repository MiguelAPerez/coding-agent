...
import { getSkills, saveSkill, deleteSkill, syncSystemSkills, importSkillFromRepo } from "@/app/actions/skills";
...
export default function SkillLibraryManager() {
    ...
    const isEditingManaged = skills.find(s => s.id === isEditing)?.isManaged;
                        ...
                        <div className="flex justify-between items-center sticky top-0 bg-background/50 backdrop-blur-md py-4 z-10 border-b border-border/30">
                            <div>
                                <h2 className="text-2xl font-black italic uppercase tracking-tight">
                                    {isEditing === "" ? "New Skill" : (isEditingManaged ? `Viewing: ${editForm.name}` : `Editing: ${editForm.name}`)}
                                    {isEditingManaged && <span className="ml-3 text-[10px] bg-primary/10 text-primary px-2 py-1 rounded not-italic font-black tracking-widest">READ ONLY</span>}
                                </h2>
                                <p className="text-xs text-foreground/40 font-medium">
                                    {isEditingManaged ? "System skills are managed by the core platform." : "Configure skill metadata and execution script"}
                                </p>
                            </div>
                            <div className="flex gap-4">
                                <button type="button" onClick={() => setIsEditing(null)} className="px-6 py-2 text-sm font-bold text-foreground/40">{isEditingManaged ? "Close" : "Cancel"}</button>
                                {!isEditingManaged && (
                                    <button type="submit" className="px-8 py-2 bg-primary text-primary-foreground rounded-xl text-sm font-black uppercase italic shadow-xl shadow-primary/20">Save Capability</button>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="space-y-8">
                                <div className="glass p-8 rounded-[2rem] border-2 border-border/50 space-y-6">
                                    <div className="space-y-4">
                                        ...
                                        <div className={`grid grid-cols-2 gap-3 p-1 bg-background/50 rounded-xl border border-border/50 ${isEditingManaged ? 'opacity-50 pointer-events-none' : ''}`}>
                                            {(['local', 'docker'] as const).map(r => (
                                                <button
                                                    key={r}
                                                    type="button"
                                                    disabled={isEditingManaged}
